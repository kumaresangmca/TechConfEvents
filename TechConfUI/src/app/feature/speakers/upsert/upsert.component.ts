...
@Component({
  selector: 'app-speaker-upsert',
  templateUrl: './upsert.component.html',
  styleUrls: ['./upsert.component.scss'],
})
export class SpeakerUpsertComponent implements OnInit, OnDestroy {
  ...
  getFormData() {
    this.subscription = this.http.get(ApiUrl.speakerById.replace('{id}', this.id.toString())).subscribe(
      (data: ResultDTO) => {
  ...
  onSubmit() {
    ...
    if(this.isEdit){
        observable = this.http.put(ApiUrl.speakerById.replace("{id}", this.id.toString()), record);
    }else{
      observable = this.http.post(ApiUrl.speakers, record);
    }
