...
export class SpeakerSessionsComponent implements OnInit {
  ...
  loadListData() {
    debugger;
    this.http.get(ApiUrl.speakerSessionsByEventId.replace("{eventId}", this.eventId.toString())).subscribe(
      (data: ResultDTO) => {
  ...
  public onDelete(speaker: SpeakerSessionsDTO) {
    ...
    if (action) {
      this.http
        .delete(ApiUrl.speakerSessionsById.replace('{id}', speaker.id.toString()))
        .subscribe(
          (data: ResultDTO) => {
