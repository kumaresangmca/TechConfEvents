...
namespace TechConf.Web.Controllers
{
    ...
    public class OrganizationController : ControllerBase
    {
        ...
        public OrganizationController(IOrganizationService<OrganizationDTO> service)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult> GetAPIKeyByCode(string code)
        {
            var resultDTO = new ResultDTO<string>();
            var data = await service.GetAPIKeyByCodeAsync(code);
            if (data == null)
            {
